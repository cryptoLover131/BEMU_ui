...
import { GTOKEN, USDC, USDT, USDT_ETH } from 'libraries/tokens'
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    if (!GTOKEN[chainId] && !USDC[chainId]) return
    const parsed = queryParametersToSwapState(query, native.symbol, GTOKEN[chainId]?.address ?? USDC[chainId]?.address)
